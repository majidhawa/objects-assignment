...
console.log( groupedCategories())
  ...
  function highScoredStudents(){
  const performedStudents=[]
  students.forEach(student=>{
    const sum = student.scores.reduce((accumulator, score) => accumulator + score, 0)
    const average =  sum/student.scores.length
    if (average>=85){
        performedStudents.push(student.name)
    }

  })
return performedStudents}

console.log( highScoredStudents())
    
    
  
//   Given an object representing a car, with properties for the make, model, year, and a method to display 
//the car's information, write a function that takes the car object and adds
// a new method to the object called age. The age method should return the current 
